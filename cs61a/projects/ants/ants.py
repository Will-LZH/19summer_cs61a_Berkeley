        ...
        if insect.is_ant:
            ...
            if isinstance(insect,QueenAnt):
                if insect.imposter == False:
                    return
            """
            if self.ant.name == 'Queen' and self.ant.imposter==True:
                self.Ant = None
            self.Ant = None
            """
...
class Bee(Insect):
    ...
    def __init__(self, armor):
        ...
        self.be_scared = False
        self.during_scared = False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        ant_place, get_range = self.place, 0
        while ant_place != hive:
            if ant_place.bees and self.min_range <= get_range <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        location_bees = self.place.bees[:]
        Ant.reduce_armor(self, amount)
        if self.armor == 0:
            for bee in location_bees:
...
class BodyguardAnt(Ant):
    ...
    # BEGIN Problem 9
    food_cost = 4
    contained_ant = None    #stores the ant it contains. It initially starts off as None
    is_container = True
    implemented = False   # Change to True to view in the GUI
    # END Problem 9
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        # BEGIN Problem 10
        "*** YOUR CODE HERE ***"
        lst_bee = self.place.bees
        lst_bee_1 = lst_bee[:]
        count = 0
        BodyguardAnt.action(self, colony)
        while count < len(lst_bee_1):
        	Insect.reduce_armor(lst_bee_1[count], self.damage)
        	count += 1
        # END Problem 10
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            return
            #imposter = False
        colony.queen = QueenPlace('Queen Place',colony)
        ScubaThrower.action(self,colony)
        test_place = self.place
        while test_place != None:
            test_place = test_place.exit
            if test_place == None:
                break
            if test_place.ant != None and test_place.ant.double ==False:
                test_place.ant.damage *= 2
                test_place.ant.double = True
            if test_place.ant != None:
                if test_place.ant.name == 'Tank' or test_place.ant.name == 'Bodyguard':
                    if type(test_place.ant.contained_ant) != 'Queen' and test_place.ant.contained_ant!= None and test_place.ant.contained_ant.double==False:
                        test_place.ant.contained_ant.damage *= 2
                        test_place.ant.contained_ant.double = True
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    action_changed = effect(bee.action, bee)
    action_origin = bee.action
    def change_action(colony):
        nonlocal duration
        if duration > 0:
            duration -= 1
            action_changed(colony)
        else:
            action_origin(colony)
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if target and not target.during_scared :
            apply_effect(make_scare, target, 2)
            target.during_scared = True
        # END Problem EC

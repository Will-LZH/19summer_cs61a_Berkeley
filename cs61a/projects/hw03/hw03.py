...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    #a,b=left(m)[1],right(m)[1]
    #print(a)
    #print(b)
    if False in [balanced(end(x)) for x in branches(m) if not is_weight(end(x))]:
        return False
    if length(branches(m)[0])*total_weight(end(branches(m)[0])) != length(branches(m)[1])*total_weight(end(branches(m)[1])):
        return False
    return True
...
def totals_tree(m):
    ...
    if is_weight(m):
        ...
        return tree(total_weight(m),[totals_tree(end(s)) for s in branches(m)])

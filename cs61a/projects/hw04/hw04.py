...
def make_withdraw(balance, password):
    ...
    count = 0
    array = []
    def withDraw(money,inputPassword):
        nonlocal inPassword,inBalance,count,array
        if count == 3:
            return 'Your account is locked. Attempts: ' + str(array)
        if inputPassword != inPassword:
            count+=1
            array.append(inputPassword)
            return 'Incorrect password'
        if inBalance-money<0:
...
class VendingMachine:
    ...
    def vend(self):
        ...
        if self.balance<self.price:
            sub = self.price-self.balance
            return "You must deposit $"+str(sub)+" more."
        elif self.balance>self.price:
            temp = self.balance-self.price
            self.balance=0
            self.stock-=1
            return "Here is your "+ self.goods+" and $"+str(temp)+" change."
        else:
            self.balance=0
            self.stock-=1
    ...
    def deposit(self,amount):
        ...
        if self.stock==0:
            temp=self.balance
            self.balance=0
            return "Machine is out of stock. Here is your $" + str(temp)+"."
...
def generate_paths(t, x):
    ...
    for a in t.branches:
        for value in generate_paths(a,x):
            yield [t.label]+valueok --
